...
/* Set gravity constant*/
  #ifdef ISOTHERMAL
   static Real Gamma = 1.0;
  #endif
  four_pi_G = 4*PI*(1/sqrt(2*PI));
  grav_mean_rho = d0;
#endif /* SELF_GRAVITY */
...
/* Now initialize solution */
  ...
  case 3:
    ...
    pGrid->U[k][j][i].d = d0*pow(cosh(x3),-2)*(1.0 + amp*sinkx);
#ifndef ISOTHERMAL
  ...
  switch(kdir){
    case 1:
    pGrid->B1i[k][j][i] = 0.0;
    pGrid->B2i[k][j][i] = b0*(1.0+ampb*sin(kwave*xl));
    pGrid->B3i[k][j][i] = 0.0;
    pGrid->U[k][j][i].B1c = 0.0;
    pGrid->U[k][j][i].B2c = b0*(1.0+ampb*sinkx);
    pGrid->U[k][j][i].B3c = 0.0;
    break;
    case 2:
    pGrid->B1i[k][j][i] = 0.0;
    pGrid->B2i[k][j][i] = 0.0;
    pGrid->B3i[k][j][i] = b0*(1.0+ampb*sin(kwave*xl));
    pGrid->U[k][j][i].B1c = 0.0;
    pGrid->U[k][j][i].B2c = 0.0;
    pGrid->U[k][j][i].B3c = b0*(1.0+ampb*sinkx);
    break;
    case 3:
    pGrid->B1i[k][j][i] = b0*(1.0+ampb*sin(kwave*xl));
    pGrid->B2i[k][j][i] = 0.0;
    pGrid->B3i[k][j][i] = 0.0;
    pGrid->U[k][j][i].B1c = b0*(1.0+ampb*sinkx);
    pGrid->U[k][j][i].B2c = 0.0;
    pGrid->U[k][j][i].B3c = 0.0;
  }
#endif /* MHD */
